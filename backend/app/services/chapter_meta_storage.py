...
CHAPTER_META_INDEX_STATEMENTS = (
    (
        "idx_chapter_meta_unique",
        ["project_id", "chapter_order"],
        "CREATE UNIQUE INDEX idx_chapter_meta_unique ON chapter_meta(project_id, chapter_order)",
    ),
    (
        "idx_chapter_meta_project",
        ["project_id", "chapter_order"],
        "CREATE INDEX idx_chapter_meta_project ON chapter_meta(project_id, chapter_order ASC)",
    ),
)
...
class ChapterMetaStorage:
    ...
    def ensure_schema(self) -> None:
        ...
        with self.connect() as connection:
            ...
            self._migrate_legacy_chapter_meta_table(connection)
            self._ensure_chapter_meta_indexes(connection)
    ...
    def _migrate_legacy_chapter_meta_table(self, connection: sqlite3.Connection) -> None:
        if "chapter_index" not in self._column_names(connection, "chapter_meta"):
            return
        migrated_table = "chapter_meta__migrated"
        connection.execute(f"DROP TABLE IF EXISTS {migrated_table}")
        connection.execute(CHAPTER_META_TABLE_SQL.replace("chapter_meta", migrated_table, 1))
        connection.execute(
            f"""
            INSERT INTO {migrated_table} (
                id, project_id, chapter_order, chapter_id, title, summary_text,
                timeline_note, start_anchor, end_anchor, open_threads_json, created_at, updated_at
            )
            SELECT
                id,
                project_id,
                COALESCE(NULLIF(chapter_order, 0), chapter_index),
                chapter_id,
                title,
                summary_text,
                timeline_note,
                COALESCE(start_anchor, ''),
                COALESCE(end_anchor, ''),
                open_threads_json,
                created_at,
                updated_at
        ...
        connection.execute("DROP TABLE chapter_meta")
        connection.execute(f"ALTER TABLE {migrated_table} RENAME TO chapter_meta")

    def _ensure_chapter_meta_indexes(self, connection: sqlite3.Connection) -> None:
        for index_name, expected_columns, create_sql in CHAPTER_META_INDEX_STATEMENTS:
            if self._index_columns(connection, index_name) == expected_columns:
                continue
            connection.execute(f"DROP INDEX IF EXISTS {index_name}")
            connection.execute(create_sql)

    def _index_columns(self, connection: sqlite3.Connection, index_name: str) -> list[str]:
