...
class WorldlineAutoEvolutionTaskService:
    ...
    async def _run_worker(
        ...
        try:
            result = await self._evolve_branch(
                task_id,
                session_id,
                project_id,
                graph_id,
    ...
    async def _evolve_branch(
        ...
        while completed_steps < max_steps:
            branch, agents, memory_hints, action_views = await self._snapshot_branch(session_id, project_id, graph_id, branch_id, goal_text)
            action_plan = self.auto_action_service.generate_actions(branch, agents, goal_text, memory_hints, action_views)
            branch = await self._apply_round(
                session_id,
                project_id,
                graph_id,
                branch_id,
            ...
            reason = stop_reason(
                completed_steps, max_steps, branch, action_plan["actions"], goal_verdict, consecutive_empty,
            )
    ...
    async def _update_processing_task(
        self,
        task_id: str,
        branch_id: str,
        branch_title: str,
