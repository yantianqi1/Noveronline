...
如果当前任务是“整次换新技术栈重构”，不要只看早期迁移设计，必须额外阅读：

- [重构总控与并行执行总览](../重构方案/00-重构总控与并行执行总览.md)
- [前端完整重构方案](../重构方案/13-前端完整重构方案.md)
- [后端完整重构方案](../重构方案/14-后端完整重构方案.md)
- [整次换新栈重构设计](./plans/2026-04-11-mirofish-novel-full-rebuild-design.md)
- [整次换新栈重构实施计划](./plans/2026-04-11-mirofish-novel-full-rebuild-plan.md)
...
执行顺序不要再沿用“先补一点后端再补一点前端”的小步修补模式，而要按总控文档中的固定顺序推进：

1. 先冻结 `06/07/08/09/10`
2. 再按 `12` 切 subagent 边界
3. 然后分别执行 `13` 和 `14`
4. 最后只在迁移 rehearsal、故障演练、回滚演练通过后切换
