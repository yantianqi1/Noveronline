...
| 档案库 archive records | `ArchiveLibraryService` 对外读的是主库 `archive_library`，但每次 list/get 前都会 `sync_incremental()`，同步源仍是各项目 `narrative_archives.json`（`backend/app/services/archive_library_service.py:80-167,169-220`；`backend/app/tables/archive.py:7-25`）。 | `archive_library` / `archive_sources` | DB 镜像 + JSON 真相 | P0 | 1) 档案生成流程直接写 `archive_library`；2) list/get 不再先扫描 `narrative_archives.json`；3) `archive_sources` 只保留导入审计，不再作为运行依赖。 | `backend/app/services/archive_library_service.py:33-35,91,136,169-193`；`backend/app/services/worldline_source_loader.py:105-112`。 |
...
| 全局搜索索引 | `GlobalSearchIndexer` 已把搜索索引落在主库 `global_index`，但索引内容来自 `UnifiedAssetView` 聚合后的多源数据，而这些源中仍含 JSON / worldlines / archive mirror（`backend/app/services/assets/global_search_indexer.py:1-10,29-55`；`backend/app/repositories/search_repo.py:1-12,35-87`）。 | `global_index`（后续再补 `global_index_fts` / pg_trgm backend） | 已统一索引表 + 上游源未统一 | P1 | 1) 上游 unified asset 来源统一后重建索引；2) 搜索后端完成 SQLite/PostgreSQL 抽象；3) 删除对 legacy source 的隐式依赖。 | 间接 legacy 来源：`backend/app/services/assets/unified_asset_view.py:3-12,611-744`；世界线文件：`backend/app/services/world_state_store.py:16-210`。 |
