...
| Phase C (部分完成) | `d020f94` ~ `f969035` (12 commits) | 2026-04-16 | 修复 46 个失败（56→10）。B1 批：`TaskManager`/`SeedExtractTaskService`/`LocalBlockFactExtractor` 等 async service 调用在 sync 测试里包 `asyncio.run`（15 文件，修 24 个）。B2 批：重写 `LlmConcurrencyService` 测试为 async（threading→asyncio.Event），LLM client snapshot/set_limit 包 asyncio.run（2 文件，修 7 个）。C 批：`LocalStoryGraphBuilder.storage→._repo`、`worldline_runtime_api` legacy sqlite3 文件直查改为 unified DB 查询、`fake_extract_batch` 改 async（3 文件，修 6 个）。D 批：`fastapi_b1_bootstrap` expected set 加 `book_plans`；`CORSMiddleware` 加 `allow_private_network=True` (Starlette 1.0+)；`worldline_prepare_service._build_dossier` source_archive_id/source_entity_uuid 用 `or ""` 防 None；`seed_runner_llm_validation` 用新 module key；`test_local_story_graph_pipeline` 查 unified DB；`chapter_meta_service.get_history_items` 补 open_thread/relationship item；`canon_history` 测试查 `get_history_items` 替代 legacy sqlite；conftest 改为 function-scoped tmp DB（解 chapter_content 单列 PK 跨 test 冲突）；service normalize_json_object 补到 sequential_reader/chapter_card_generator/character_agent_profile_generator（修 8 个）。FTS：`assets_fts` 加 INSERT/UPDATE/DELETE trigger（修 2 个）。**剩 10 个失败**：4 worldline_prepare（TestClient 下 async background task 调度 flaky）、4 业务断言（long_novel/offline_novel/seed_llm_payload_normalization/async_seed_pipeline timing）、1 order-sensitive graph_build_task_state、1 order-sensitive local_story_graph_pipeline。安全锚：`pre-phase-c-20260416`。 |
| Phase C 续（剩余 10 failed） | _未开始_ | | 下会话攻 |
