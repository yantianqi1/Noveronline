...
| Phase C (部分完成) | `d020f94` ~ `3b49418` (7 commits) | 2026-04-16 | 修复 45 个失败（56→11-12）。B1 批：`TaskManager`/`SeedExtractTaskService`/`LocalBlockFactExtractor` 等 async service 调用在 sync 测试里包 `asyncio.run`（15 文件，修 24 个）。B2 批：重写 `LlmConcurrencyService` 测试为 async（threading→asyncio.Event），LLM client snapshot/set_limit 包 asyncio.run（2 文件，修 7 个）。C 批：`LocalStoryGraphBuilder.storage→._repo`、`worldline_runtime_api` legacy sqlite3 文件直查改为 unified DB 查询、`fake_extract_batch` 改 async（3 文件，修 6 个）。D 批：`fastapi_b1_bootstrap` expected set 加 `book_plans`；`CORSMiddleware` 加 `allow_private_network=True` (Starlette 1.0+)；`worldline_prepare_service._build_dossier` source_archive_id/source_entity_uuid 用 `or ""` 防 None；`seed_runner_llm_validation` 用新 module key；`test_local_story_graph_pipeline` 查 unified DB（6 files，修 7 个）。FTS：`assets_fts` 加 INSERT/UPDATE/DELETE trigger，修好 manuscript_service 2 个 FTS 搜索（修 2 个）。**剩 11-12 个失败**：4 个 worldline_prepare（TestClient + async background task lifecycle 复杂问题）、1 个 graph_build_task_state（order-sensitive，独立跑通过）、2 个 chapter_context_pack_builder（同 order 问题）、5 个桶 D 业务断言（canon_history / async_seed_pipeline / long_novel / offline_novel / seed_llm_payload_norm）。安全锚：`pre-phase-c-20260416`。 |
| Phase C 续（剩余 11-12 failed） | _未开始_ | | 下会话攻 |
